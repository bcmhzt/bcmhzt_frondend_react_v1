...
export interface usePostReplyImageUploaderOptions {
  baseDir?: string;
  uid?: string;
  limit?: number;
}
...
export function usePostReplyImageUploader(
  options: usePostReplyImageUploaderOptions = {}
) {
  const { baseDir = 'reply_uploads', uid = '', limit = 10 } = options;
  const [files, setFiles] = useState<File[]>([]);
  const [uploadProgress, setUploadProgress] = useState<number[]>([]);
  const [urls, setUrls] = useState<string[]>([]);
