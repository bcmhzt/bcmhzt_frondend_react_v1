...
const Register = () => {
  ...
  useEffect(() => {
    ...
    const checkReferralHash = async () => {
      ...
      try {
        if (debug === 'true') {
          console.log('[src/pages/Register.js] checking referralHash:', referralHash);
        }
        ...
        if (debug === 'true') {
          console.log('[src/pages/v1/Register.js] checkReferralHash response true:', response.data);
          if (
            response.data.data &&
            ...
            console.log('[src/pages/v1/Register.js] introduced_hash:', response.data.data[0].introduced_hash);
          }
        }
       ...
       if (response.data.status === 201) {
        ...
        if (err.response && err.response.status === 429) {
          console.log('[src/pages/v1/Register.js] checkReferralHash response:', err.response.data);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (debug === 'true') {
        console.log('[src/pages/Register.js:119] Response from /user/verifyemail:', response);
      }

      if (response && response.status === 201) {
        const currentLang = i18n.language;
        if (debug === 'true') {
          console.log('[src/pages/Register.js:109] Success: ' + response.data.hash);
        }
      ...
      <div className="card p-4" style={{ width: '100%', maxWidth: '400px' }}>
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="form-group mb-3">
            <label className="mb10" htmlFor="email">{t('register.email')}</label>
            <input
              type="email"
              id="email"
