...
const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, index: number) => {
  ...
  const handleVerify = async () => {
    ...
    if (verificationCode.length !== 6) {
      setIsLoading(false);
      setError(t('verifyemail.input_verification_code'));
      return;
    }
    if (password === null || password === '') {
      setIsLoading(false);
      setError(t('verifyemail.input_password'));
      return;
    }
    if (password !== confirmPassword) {
      setIsLoading(false);
      setError(t('verifyemail.input_password_not_match'));
      return;
    }
    ...
    if (!passwordPolicy.test(password)) {
      setIsLoading(false);
      setError(t('verifyemail.input_password_policy'));
    return;
  }
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        console.log("[src/pages/VerifyEmail.js:124] /user/verifiedemail error:", error.response?.status);
      } else {
        console.log("[src/pages/VerifyEmail.js:124] /user/verifiedemail error:", error);
      }
      ...
      <div className="login container d-flex justify-content-center mt20">
        <div className="card p-2 verify-email" style={{ width: '100%', maxWidth: '400px' }}>
          <div className="card-body mt20">
            ...
            {/* エラーメッセージ表示 */}
            {error && <p className="text-danger mt20 ml50 mr50 d-flex align-items-end">{error}</p>}
            <p className="card-text description">
              {t('verifyemail.message')}<br></br>
            </p>
            <div className="d-flex justify-content-center">
              {code.map((digit, index) => (
                <input
                  ...
                  style={{ width: '40px', margin: '0 5px', textAlign: 'center', fontSize: '24px' }}
                />
              ))}
            </div>
