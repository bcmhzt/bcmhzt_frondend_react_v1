...
if (debug === 'true') console.log('[BHeight.tsx] debug:', debug);
...
const BHeight = () => {
  ...
  const [value, setValue] = useState(
    currentUserProfile.user_detail.bheight || ''
  );
  ...
  const handleBlur = async () => {
    ...
    try {
      const res = await axios.post(
        `${apiEndpoint}/update/user/detail/bheight/${currentUserProfile.user_profile.uid}`,
        { value, token },
      ...
      console.error('API error [BHeight]:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="profile-update bheight">
      <h3>身長</h3>
