...
const MyPropensitiesBase = () => {
  ...
  const handleInputChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    console.log(
      '[src/components/users/MyPropensitiesBase.tsx:30] ✅️ e.target.value:',
      e.target.value
    );
  ...
  const fetchPropensityTagsFromApi = async (keyword: string) => {
    try {
      const response = await axios.post(
        `${apiEndpoint}/v1/get/propensity_tags_base`,
        { words: keyword },
        {
      ...
      console.log(
        '[src/components/users/MyPropensitiesBase.tsx:74] ✅ fetchPropensityTagsFromApi API response:',
        response.data.data.data
      );
      setPropensityTags(response.data.data.data);
      // setPropensityTags([]);
    } catch (error) {
      console.error('[MyPropensitiesBase] ❌ API error:', error);
  ...
  const handleSearch = () => {
    ...
    console.log(
      '[src/components/users/MyPropensitiesBase.tsx:42] 🌼 e.target.value:',
      inputKeyword
    );
  };
  ...
  const handleTagClick = async (propensity_id: number) => {
    console.log(
      '[src/components/users/MyPropensitiesBase.tsx:99] 🌼 handleTagClick called with propensity_id:',
      propensity_id
    );
    try {
      const response = await axios.post(
      ...
      console.log(
        '[src/components/users/MyPropensitiesBase.tsx:114] ✅ handleTagClick API response:',
        response.data
      );
      // ✅ 即時UI反映
      ...
      console.error(
        '[src/components/users/MyPropensitiesBase.tsx:106] ❌ handleTagClick API error:',
        error
      );
    }
  };
  ...
  const handleSelectMyPropensityTag = async (checked: boolean) => {
    ...
    console.log(
      '[src/components/users/MyPropensitiesBase.tsx:114] ✅ handleSelectMyPropensityTag called with checked:',
  ...
  return (
    <>
      <div className="propensity-tags mb50 mt20">
        <h3>
          性癖<span className="subtitle">base</span>
        </h3>
