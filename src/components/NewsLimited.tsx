 ...
 * type: page | component
 * description: NewsLimitedはmicrocmsのニュースから最新5件を取得し表示する
 * 
 */
...
const NewsLimited = () => {
  ...
  if (!mcmcServiceDomain || !mcmcApikey) {
    throw new Error("MICROCMS_SERVICE_DOMAIN and MICROCMS_APIKEY must be defined in environment variables.");
  }
  const client = useRef(
    createClient({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("[src/pages/Top.js:53] Error fetching API response:", error);
      }
    };
    fetchData();
  }, [client]);
  ...
  return (
    ...
    <div className="news">
      <h2 className="title mb30">
        ...
        <span className="subtitle">news</span></h2>
          ...
          <React.Fragment key={value.id || idx}>
            <div className="news-item">
              <div id={value.id.toString()}>
                ...
                <span className="news-title">{value.title.toString()}: </span>
                <span>
                  {value.message.replace(/<\/?[^>]+(>|$)/g, "").length > 250
                    ? value.message.replace(/<\/?[^>]+(>|$)/g, "").slice(0, 250) + " ... "
